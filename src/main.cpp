...
#if defined(NDEBUG)
# error "Metticoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x727bba987ecbe0eb6238f985aab6c8df4b36c74a6f5a83796b96eff4899c589e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Metticoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Metticoin Signed Message:\n";
...
{
    // Metticoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Metticoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Metticoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Metticoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Metticoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Metticoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x01947b912de18776272c3db7cc4d9504026c0adb0e691a85fe7cbf147afef206");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Metti hat einen kleinen";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0404b82a88a1652203f3a1066d7149428465a3a5c8968d910fab55649b4fb2f907cc576cc9079b210303921099b8401a40353efd007e009323a742f777b0484da8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516525395;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084644848;

        if (fTestNet)
        {
            block.nTime    = 1516525365;
            block.nNonce   = 385509145;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("321df3bb9fee7a852db1e53f179489d1f05629432235d2395315a19329668786"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Metticoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MetticoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MetticoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MetticoinMiner(CWallet *pwallet)
{
    printf("MetticoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("metticoin-miner");
    ...
    try { loop {
        ...
        printf("Running MetticoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MetticoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MetticoinMiner, pwallet));
}
